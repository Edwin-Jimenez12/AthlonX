...
export function ContextSwitcher({ contexts, activeContextId, onChange }: ContextSwitcherProps) {
  ...
  return <div className="relative">
    <button type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} className="flex max-w-64 cursor-pointer items-center gap-2 rounded-[5px] border border-[#294052] px-2 py-1.5 text-left text-white transition hover:border-[#b4ff45]">
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[5px] bg-[#b4ff45] text-xs font-bold text-[#07131e]">{activeContext.name.slice(0, 2).toUpperCase()}</span>
