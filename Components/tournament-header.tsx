...
type TournamentHeaderProps = { activeSection: 'Resumen' | 'Equipos' | 'Partidos' | 'Puntajes' | 'Fixtures' }

export default function TournamentHeader({ activeSection }: TournamentHeaderProps) {
  const sections = ['Resumen', 'Equipos', 'Partidos', 'Puntajes', 'Fixtures'] as const
  ...
  return (
    <header className="rounded-2xl bg-[#081522] p-6 text-white shadow-sm md:p-8">
      ...
      <nav className="mt-6 flex flex-wrap gap-2 text-sm font-semibold">{sections.map((section) => <Link key={section} href={section === 'Equipos' ? '/dashboard/torneos/liga-panamena-rugby/equipos' : section === 'Partidos' ? '/dashboard/torneos/liga-panamena-rugby/partidos' : section === 'Puntajes' ? '/dashboard/torneos/liga-panamena-rugby/puntajes' : section === 'Fixtures' ? '/dashboard/torneos/liga-panamena-rugby/fixtures' : '/dashboard/torneos/liga-panamena-rugby'} className={`rounded-full border px-4 py-2 ${activeSection === section ? 'border-[#B4FF45] bg-[#B4FF45] text-[#081522]' : 'border-white/20 text-slate-200 hover:border-[#B4FF45] hover:text-[#B4FF45]'}`}>{section}</Link>)}</nav>
    </header>
  )
}
